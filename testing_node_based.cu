...
int main() {
    ...
    int* h_labels;       // [num_nodes]
     ...
     // Load features
    load_features("/kaggle/input/citeseer/features.txt", &h_features, num_nodes, input_dim);

    // Load row_ptr
    int row_ptr_len;
    load_int_array("/kaggle/input/citeseer/row_ptr.txt", &h_row_ptr, row_ptr_len);
    ...
    // Load col_idx
    int col_idx_len;
    load_int_array("/kaggle/input/citeseer/col_idx.txt", &h_col_idx, col_idx_len);
    num_edges = col_idx_len;

    //load labels
    int labels_len;
    load_int_array("/kaggle/input/citeseer/labels.txt", &h_labels, labels_len);
    ...
    const int L = 2;           // number of GATv2 layers
    int head[L] = {4, 1};         // Number of attention-heads per layer
    int out_dim[L] = {32, 16};      // Output dim per head per layer
    bool weight_sharing = false; // Whether to share weights across layers
    ...
    int in_dim[L]; // Input dim for first layer, subsequent layers will be computed based on previous layer's output.
    in_dim[0] = input_dim;
    for (int l = 1; l < L; ++l)
    ...
    float* d_H[L];          // Output buffers per layer post-nonlinearity
    ...
    float* d_attn_score[L]; // Attention scores per layer [num_heads][edges]
    float* d_attn_coeff[L]; // Attention coefficients per layer [num_heads][edges]
    ...
    int wt_offset[L];   // Offset for each layer's weights in the flat array
    int attn_offset[L]; // Offset for each layer's attention vectors in the flat array
    ...
    float* input_gradients[L];     // Array of pointers to output gradients per layer//these are the gradients which are coming out of that layer which will become as input gradient to previous layer
    ...
    total_w = 0;
    total_a = 0;
    wt_offset[L]= {0};  // it will have how much size consumed till previous layer
    attn_offset[L]= {0}; // it will have how much size consumed till previous layer
    ...
    printf("Total size of learnable parameters for all layers: %zu MB\n", (total_w+ total_a + (C*out_dim[L-1]) * sizeof(float)) / (1024 * 1024));
    // Start timer
    ...
    setup_states_kernel<<<gridSize, blockSize>>>(d_states, 12345);  // Seed with fixed value. //TAKE time(NULL) for random seed
