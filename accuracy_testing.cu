...
__global__ void gatv2_layer_backward(
    ...
    //========= LOAD dL_d_alpha_ij and dL_d_e_ij to device memory============
    attn_score_grad[(i * num_heads + h) * max_degree + tid] = dL_d_e_ij;
    //===============================================================
    ...
    // ---- DIRECT GRADIENT FOR x_i (ASSUMED GRAPH IS UNDIRECTED) ----
    // For node i as a neighbor of node j, accumulate to grad_x_lower[i][*] as per direct formula
    int offset = -1;   //(will create problem if directed edge)
    for (int t = d_row_ptr[j]; t < d_row_ptr[j+1]; ++t) {
        if (d_col_idx[t] == i) {
    ...
    //===============DEBUG PRINT OF DIRECT GRADIENT FOR x_i================
    // if(i==0 && tid==0){
    //     printf("\nDirect gradient for x_i computed for node 1\n");
    //     for(int idx=0; idx<in_dim; ++idx){
    //        printf("%f ", grad_x_lower[i* in_dim+ idx]);
    //     }
    //     printf("\n");
    // }
    //=====================================================================

    float grad_e_ji = attn_score_grad[(j * num_heads + h) * max_degree + offset];
    int leaky_base_ji = (((j * num_heads + h) * max_degree) + offset) * out_dim;
    float* s_ji = &d_s[leaky_base_ji]; // each [out_dim]


    // ---- INDIRECT GRADIENT FOR x_i ----
    //case-1: when i is the neighbour of j. i xi changes then e_ji changes hence loss changes
    for (int od = 0; od < out_dim; ++od) {
        // Compute LeakyReLU' for this dimension
        float leaky_grad = (s_ji[od] > 0) ? 1.0f : negative_slope;
        // Compute elementwise product with a
        float elem = d_a[h * out_dim + od] * leaky_grad * grad_e_ji;

        // right part of W for this head/output: [out_dim][in_dim]
        for (int id = 0; id < in_dim; ++id) {
            float W_right = d_w[(h * out_dim * 2 * in_dim) + (od * 2 * in_dim) + (in_dim + id)];
            float grad_contrib = elem * W_right;
            atomicAdd(&grad_x_lower[i * in_dim + id], grad_contrib);
        }
    }

    //case-2: when i is the center node and j is the neighbour of i.  xi changes then e_ij changes hence loss changes. we use W-right now
    if(j!=i){      //avoiding self-loop double counting
        for (int od = 0; od < out_dim; ++od) {
            // Compute LeakyReLU' for this dimension
            float leaky_grad = (s_ij[od] > 0) ? 1.0f : negative_slope;
            // Compute elementwise product with a
...
int main() {
    ...
    int out_dim[L] = {100, 7};    // Output dim per head per layer
    // int out_dim[L];
    ...
    for (int epoch = 1; epoch <=50; ++epoch) {
        ...
        for (int l = L-1; l >= 0; --l) {
            ...
            float* attn_score_grad;
            cudaMalloc(&attn_score_grad, num_nodes * head[l] * max_degree * sizeof(float));
            ...
            grad_output, negative_slope, max_degree, is_last_layer, l, attn_score_grad);
